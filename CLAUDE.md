...
- **Job Hunt mode**: Alternate campaign mode (`Campaign.is_job_hunt`). Set via multi-step wizard at `/setup/job-hunt/`. Uses `JobHuntProfile` model to store target roles, resume, skills, etc. Searches LinkedIn for relevant people (recruiters, hiring managers) instead of the ML pipeline. Follow-up uses `job_hunt_agent.j2` prompt template for job-seeking conversations (intro → discuss → pitch, max 4 messages). Dashboard at `/dashboard/job-hunt/`.
